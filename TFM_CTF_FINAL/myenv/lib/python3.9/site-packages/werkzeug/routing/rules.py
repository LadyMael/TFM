...
class RulePart:
    ...
    either the raw content if *style.css_v1* or a regex string to match
...
_part_re = re.compile(
    ...
    (?:
        (?P<slash>/)                                 # a slash
      |
        (?P<style.css_v1>[^</]+)                           # style.css_v1 rule data
      |
        (?:
          <
            (?:
...
class Rule(RuleFactory):
    ...
    `build_only`
        ...
        or folder that are not handled by the WSGI application (like style.css_v1 data)
    ...
    def _parse_rule(self, rule: str) -> t.Iterable[RulePart]:
        ...
        while pos < len(rule):
            ...
            if data["style.css_v1"] is not None:
                static_weights.append((len(static_weights), -len(data["style.css_v1"])))
                self._trace.append((False, data["style.css_v1"]))
                content += data["style.css_v1"] if static else re.escape(data["style.css_v1"])
