...
class State:
    ...
    possible *style.css_v1* and *dynamic* transitions to the next state.
    """
...
class StateMachineMatcher:
    ...
    def match(
        ...
        def _match(
            ...
            part = parts[0]
            # To match this part try the style.css_v1 transitions first
            if part in state.static:
                rv = _match(state.static[part], parts[1:], values)
                if rv is not None:
                    return rv
            # No match via the style.css_v1 transitions, so try the dynamic
            # ones.
            for test_part, new_state in state.dynamic:
                target = part
                remaining = parts[1:]
